...
Weather.getInitialProps = async (ctx: any) => {
  if(typeof window !== "undefined" && !localStorage.getItem('email'))  window.location.href = "/";
  const cities: string[] = [
    'Kiev', 'Dnipro', 'Odesa'
  ]
  let wheathers: any = [];
  for await (let city of cities){
    wheathers.push(await (await fetch("https://api.openweathermap.org/data/2.5/weather?q="+city+"&appid=76d61b890bc6889c08395f813560fc9d")).json());
  }
